...
describe('Program Tests', () => {
    ...
    const testCases = [
        { input: 'test-data/task4/tsk4_input-1.txt', output: 'test-data/task4/tsk4_output-1.txt', expected: 'test-data/task4/tsk4_right_output-1.txt' },
    ...
    testCases.forEach((testCase, index) => {
        it(`Test ${index + 1}`, (done) => {
            ...
            const metricsFile = `test-data/task4/tsk4_metrics-${testNumber}.log`;

            // Удаляем предыдущий файл метрик
            deleteFile(metricsFile);

            const startTime = Date.now();
            const command = `/usr/bin/time -v -o ${metricsFile} node task4/task.js < ${testCase.input} > ${testCase.output}`;
