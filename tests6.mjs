...
describe('Program Tests', () => {
    ...
    const testCases = [
        { input: 'test-data/task6/tsk6_input-1.txt', output: 'test-data/task6/tsk6_output-1.txt', expected: 'test-data/task6/tsk6_right_output-1.txt' },
    ...
    testCases.forEach((testCase, index) => {
        it(`Test ${index + 1}`, (done) => {
            ...
            const metricsFile = `test-data/task6/tsk6_metrics-${testNumber}.log`;

            // Удаляем предыдущий файл метрик
            deleteFile(metricsFile);

            const startTime = Date.now();
            const command = `/usr/bin/time -v -o ${metricsFile} node task6/task.js < ${testCase.input} > ${testCase.output}`;
            ...
            exec(command, (error, stdout, stderr) => {
                ...
                try {
                    ...
                    if (memoryMatch) {
                        memoryMB = (parseInt(memoryMatch[1], 10) / 1026).toFixed(2);
                    }
                } catch (e) {
