...
describe('Program Tests', () => {
    ...
    const testCases = [
        { input: 'test-data/task3/tsk3_input-1.txt', output: 'test-data/task3/tsk3_output-1.txt', expected: 'test-data/task3/tsk3_right_output-1.txt' },
    ...
    testCases.forEach((testCase, index) => {
        it(`Test ${index + 1}`, (done) => {
            ...
            const metricsFile = `test-data/task3/tsk3_metrics-${testNumber}.log`;

            // Удаляем предыдущий файл метрик
            deleteFile(metricsFile);

            const startTime = Date.now();
            const command = `/usr/bin/time -v -o ${metricsFile} node task3/task.js < ${testCase.input} > ${testCase.output}`;
